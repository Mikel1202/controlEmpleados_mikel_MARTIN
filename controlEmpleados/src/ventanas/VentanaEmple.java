...
JLabel titulo=new JLabel ("GESTI�N DE EMPLEADOS. BD QLITE");
...
{ 	
    setModal(true);
    setTitle("GESTI�N DE EMPLEADOS. BD QLITE ejemplo");
	...
	if (e.getSource() == balta) { //SE PULSA EL BOTON alta   	
		...
		try{
			...
			grabar();
				...
				mensaje.setText("N�mero de empleado incorrecto: " +num.getText() ); 
			}
  ...
  try
     ...
     //e.printStackTrace();
 	 if (e.getErrorCode()==17002)
        mensaje.setText("Error NO SE PUEDE ESTABLECER CONEXI�N CON LA BD. Revisa la conexi�n.");   
...
{	int error=0;
	...
	try{
		dire=Integer.parseInt(listadirec.getSelectedItem());
			...
			mensaje.setText("N�mero de director incorrecto: " + listadirec.getSelectedItem() ); 
			error=1;}
	try{
	...
	try{
		...
		try{
			DEPT_NO=Integer.parseInt(listadepar.getSelectedItem());
				...
				mensaje.setText("N�mero de departamento incorrecto: " + listadepar.getSelectedItem() ); 
				error=1;}
	if (error!=1)
...
public void cargalistaBDdirec(Choice listadirec,JLabel nombredir, String[] nombredirec)
{
	...
	while (resul.next())
	{
	  listadirec.add(Integer.toString(resul.getInt(1))); // guardo n�m de empleado
	  // guardo el nombre en un array seg�n la posici�n del elemento, la posici�n la dice emp 
...
public void cargalistaBD(Choice lista, JLabel nomd, String[] nombredepart2) {
	try
		...
		while (resul.next())
		{
		   lista.add(Integer.toString(resul.getInt(1))); // guardo n�m de depar en la lista
		   // guardo el nombre en un array seg�n la posici�n del elemento, la posici�n la dice dep 
